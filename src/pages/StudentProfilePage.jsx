        ...
        <div className="relative group w-full mt-3">
            ...
            <div className="relative w-full bg-slate-200/50 dark:bg-slate-700/50 rounded-full h-2.5 overflow-hidden shadow-inner border border-white/50 dark:border-white/5">
                <div className="h-full rounded-full transition-all duration-700 ease-out relative overflow-hidden" style={{ width: `${percentage}%` }}>
                    <div className="absolute inset-0 bg-gradient-to-r from-blue-500 via-indigo-500 to-indigo-500 xp-bar-glow"></div>
                </div>
            </div>
        </div>
    );
                ...
                <div className="relative z-10 w-full max-w-[1920px] mx-auto">
                    ...
                    <div className="relative w-full h-64 md:h-96 overflow-hidden rounded-b-[3rem] shadow-2xl group">
                        ...
                        {userProfile?.canUploadCover && userProfile?.coverPhotoURL ? (
                            ...
                            <div className="absolute inset-0 bg-gradient-to-br from-indigo-500 via-indigo-500 to-pink-500 flex items-center justify-center">
                ...
                <div className="relative z-10 w-full max-w-[1920px] mx-auto py-10 px-4 sm:px-8">
                    <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
                        ...
                        <aside className="lg:col-span-4 xl:col-span-3 space-y-6 lg:sticky lg:top-28 self-start">
                            ...
                            {/* Parent Link Code */}
                            <div style={monetStyle} className="glass-panel rounded-[2rem] p-5 transition-colors duration-500">
                                <div className="flex items-center gap-3 mb-3">
                                    <div className="p-2 bg-indigo-50 dark:bg-indigo-900/20 rounded-lg text-indigo-500">
                                        <LinkIcon className="w-4 h-4" />
                                    </div>
                                    <span className="text-sm font-bold text-slate-700 dark:text-slate-300">Parent Link Code</span>
                                </div>
                                <p className="text-[10px] text-slate-400 mb-3 leading-relaxed">
                                    Share this code with your parent/guardian so they can view your grades and activity.
                                </p>
                                {userProfile?.parentLinkCode ? (
                                    <div className="flex items-center gap-2">
                                        <div className="flex-1 bg-slate-100 dark:bg-slate-800 rounded-xl px-4 py-3 text-center">
                                            <span className="text-lg font-black tracking-[0.3em] text-slate-800 dark:text-white">
                                                {userProfile.parentLinkCode}
                                            </span>
                                        </div>
                                        <button
                                            onClick={() => {
                                                navigator.clipboard.writeText(userProfile.parentLinkCode);
                                                showToast('Code copied!', 'success');
                                            }}
                                            className="px-3 py-3 rounded-xl bg-indigo-50 dark:bg-indigo-900/20 text-indigo-600 dark:text-indigo-400 hover:bg-indigo-100 dark:hover:bg-indigo-900/40 transition-colors text-xs font-bold"
                                        >
                                            Copy
                                        </button>
                                    </div>
                                    ...
                                    <button
                                        onClick={async () => {
                                            try {
                                                const { generateParentLinkCode } = await import('../services/firestoreService');
                                                await generateParentLinkCode(userProfile.id);
                                                await refreshUserProfile();
                                                showToast('Parent link code generated!', 'success');
                                            } catch (err) {
                                                showToast('Failed to generate code.', 'error');
                                            }
                                        }}
                                        className="w-full py-3 rounded-xl bg-indigo-50 dark:bg-indigo-900/20 text-indigo-600 dark:text-indigo-400 hover:bg-indigo-100 dark:hover:bg-indigo-900/40 transition-colors text-xs font-bold"
                                    >
                                        Generate Code
                                    </button>
                                )}
