...
export default function ViewQuizModal({ isOpen, onClose, quiz, userProfile, classId }) {
  ...
  const fetchSubmission = useCallback(async () => {
    ...
    setLatestSubmission(submissions[0] || null);  // most recent score
    setAttemptsTaken(submissions.length);         // how many times student answered
    setLoading(false);
  ...
  const handleSubmit = async () => {
    ...
    try {
      const submissionRef = doc(collection(db, 'quizSubmissions')); // new doc every time
      ...
      await fetchSubmission(); // refresh state
    } catch (error) {
            ...
            {question.options.map((option, idx) => (
              <label key={idx} className="flex items-center space-x-2 cursor-pointer">
                <input
                  ...
                  onChange={() => setUserAnswers({ ...userAnswers, [currentQ]: idx })}
                />
                <span>{option}</span>
              </label>
            ))}
