...
// --- [MODIFIED] Using monetTheme instead of static fallback ---
                    ...
                    {post.images && post.images.length > 1 ? (
                      <div className={`grid gap-0.5 ${post.images.length === 2 ? 'grid-cols-2' :
                        ...
                        {post.images.slice(0, 4).map((img, idx) => (
                          <div
                            key={idx}
                            className={`relative overflow-hidden bg-slate-100 dark:bg-white/5 ${post.images.length === 3 && idx === 0 ? 'row-span-2 h-full' : 'aspect-square'
                              }`}
                          >
                            <img
                              src={img}
