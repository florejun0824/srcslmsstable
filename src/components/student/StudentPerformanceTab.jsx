...
const StatCard = ({ icon: Icon, title, value, unit }) => (
  <div className="bg-neumorphic-base p-4 rounded-2xl shadow-neumorphic hover:shadow-neumorphic-inset transition-shadow duration-300">
    <div className="flex items-center gap-3">
      <div className="w-10 h-10 bg-neumorphic-base shadow-neumorphic rounded-lg flex items-center justify-center">
        <Icon className="w-5 h-5 text-red-500" />
      </div>
      <div>
        <p className="text-slate-500 text-sm font-medium">{title}</p>
        <span className="text-2xl font-bold text-slate-800">{value}</span>
        {unit && <span className="text-lg font-semibold text-slate-800">{unit}</span>}
      </div>
    </div>
  </div>
);
...
const StudentPerformanceTab = ({ userProfile, classes }) => {
  ...
  return (
    <div className="space-y-6">
      {/* Header */}
      <div>
        <h1 className="text-2xl sm:text-3xl font-bold text-slate-900 tracking-tight">Performance Overview</h1>
      ...
      {/* Submissions List */}
      <div className="bg-neumorphic-base p-3 sm:p-5 rounded-2xl shadow-neumorphic">
        ...
        {submissions.length > 0 ? (
          <div className="space-y-3">
            {submissions.map((sub) => {
              ...
              return (
                <div
                  key={sub.id}
                  className="bg-neumorphic-base p-3 rounded-lg shadow-neumorphic hover:shadow-neumorphic-inset transition-all duration-300 cursor-pointer"
                >
                  <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-2">
                    <div className="flex-1">
                      <p className="font-bold text-slate-800 text-base sm:text-lg">{sub.quizTitle}</p>
                      <p className="text-xs sm:text-sm text-slate-500">{sub.className}</p>
                    </div>
                    <div className="w-full sm:w-auto flex justify-between sm:justify-end items-center gap-4">
                      <p className={`text-lg sm:text-xl font-bold ${scoreColor}`}>
                        {sub.score}/{sub.totalItems}
                        <span className="text-xs sm:text-sm font-medium ml-1 opacity-80">
            ...
            {hasMore && (
              <div className="pt-3 flex justify-center">
                <button
                  ...
                  className="bg-neumorphic-base text-slate-700 font-semibold px-4 py-2 rounded-lg shadow-neumorphic hover:shadow-neumorphic-inset transition-all duration-300 disabled:opacity-60 disabled:cursor-not-allowed"
                >
          ...
          <div className="text-center py-8 px-4 bg-neumorphic-base rounded-lg shadow-neumorphic-inset">
            <p className="text-slate-500 text-sm">You haven't submitted any quizzes yet.</p>
            <p className="text-slate-400 text-xs mt-1">Once you do, your results will appear here!</p>
          </div>
        )}
      </div>
    </div>
