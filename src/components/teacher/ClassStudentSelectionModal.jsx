...
const ClassStudentSelectionModal = ({ isOpen, onClose, onConfirm, allClasses = [], currentSelectionMap, db }) => {
    ...
    // [NEW] Mobile View State: 'classes' or 'students'
    ...
    useEffect(() => {
        if (isOpen) {
            ...
            setStudents([]);
            setSearchTerm('');
            setMobileView('classes'); // Reset to class list on open
        }
    ...
    const handleClassClick = (classId) => {
        ...
        // On mobile, automatically switch to student view
        setMobileView('students');
    };

    const handleToggleClass = async (classId, e) => {
        // Stop propagation so we don't trigger the "Enter Class" click
        e?.stopPropagation();
    ...
    const classListContent = useMemo(() => {
        ...
        return filteredClasses.map(cls => {
            ...
            return (
                ...
                >
                    <div className="flex-shrink-0" onClick={(e) => handleToggleClass(cls.value, e)}>
                        {isLoading ? (
    ...
    const studentListContent = useMemo(() => {
        ...
        // On desktop, we want to show "Select a Class". On Mobile, we won't see this because the column is hidden until selected.
        if (!activeClassId) {
        ...
        >
            <div className="relative h-[85vh] flex flex-col bg-transparent rounded-b-[2rem]">
                ...
                <main className="flex-grow flex flex-col lg:flex-row gap-6 min-h-0 p-6 sm:p-8 overflow-hidden">
                    ...
                    {/* [MOBILE LOGIC]
                      We use basic CSS display toggles based on 'mobileView' state for mobile.
                      For Desktop (lg:), we force display:flex and correct widths.
                    */}

                    {/* Column 1: Search (Hidden on Mobile if viewing Students) */}
                    ...
                    {/* Column 2: Class list (Hidden on Mobile if viewing Students) */}
                    ...
                    {/* Column 3: Student list (Hidden on Mobile if viewing Classes) */}
