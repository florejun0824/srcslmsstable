    ...
    return (
        <Dialog open={isOpen} onClose={onClose} className="fixed inset-0 z-50 flex items-center justify-center p-2 sm:p-4">
            ...
            <Dialog.Panel className="relative bg-slate-50 rounded-2xl shadow-2xl w-full max-w-4xl z-10 flex flex-col max-h-[95vh] overflow-hidden">
                ...
                {/* ✅ FIXED: Responsive padding and font size for the header */}
                ...
                {/* ✅ FIXED: Responsive padding for content area */}
                    ...
                    {currentPage === 0 && objectives.length > 0 && (
                        <div className="mb-6 p-3 sm:p-4 bg-indigo-50 border-l-4 border-indigo-300 rounded-r-lg">
                            <h3 className="flex items-center gap-2 text-base sm:text-lg font-bold text-indigo-800 mb-2">
                                <ListBulletIcon className="h-5 w-5 sm:h-6 sm:h-6" />
                                {objectivesLabel}
                            </h3>
                            {/* ✅ FIXED: Smaller font size for objectives list */}
                ...
                {/* ✅ FIXED: Responsive padding and font/button sizes in the footer */}
                <div className="flex justify-between items-center p-3 sm:p-5 bg-slate-50/80 backdrop-blur-sm border-t border-slate-200 flex-shrink-0">
                    <button
                        ...
                        className="inline-flex items-center gap-2 px-3 py-1.5 sm:px-4 sm:py-2 text-xs sm:text-sm font-semibold text-slate-600 bg-white border border-slate-300 rounded-lg shadow-sm hover:bg-slate-100 disabled:opacity-50 transition-colors"
                    >
                    ...
                    {currentPage < totalPages - 1 ? (
                        <button
                            ...
                            className="inline-flex items-center gap-2 px-3 py-1.5 sm:px-4 sm:py-2 text-xs sm:text-sm font-semibold text-white bg-indigo-600 rounded-lg shadow-sm hover:bg-indigo-700 disabled:opacity-50 transition-colors"
                        >
                            Next
                            <ArrowRightIcon className="h-4 w-4 sm:h-5 sm:h-5" />
                        </button>
                    ) : (
                        <button 
                            onClick={onClose} 
                            className="inline-flex items-center px-4 py-1.5 sm:px-5 sm:py-2 text-xs sm:text-sm font-semibold text-white bg-green-600 rounded-lg shadow-sm hover:bg-green-700 transition-colors"
                        >
                            Finish
                        </button>
                    )}
