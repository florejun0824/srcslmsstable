...
// Animation variants are unchanged as they are already optimized for a smooth experience
const modalVariants = {
    ...
    return (
        <Dialog open={isOpen} onClose={onClose} className={`fixed inset-0 z-50 flex items-center justify-center font-sans ${className}`}>
            ...
            <Dialog.Panel
                ...
                animate="visible"
                exit="exit"
                className="relative bg-gray-50 rounded-2xl shadow-2xl w-full max-w-5xl z-10 flex flex-col h-full md:h-[90vh] md:max-h-[700px] overflow-hidden border border-white/20"
            >
                {/* Progress Bar with theme color */}
                <div className="w-full bg-gray-200/50 h-1.5 flex-shrink-0">
                    <div className="bg-red-600 h-1.5 transition-all duration-500 ease-out" style={{ width: `${progressPercentage}%` }} />
                </div>
                ...
                <header className="flex justify-between items-center p-4 sm:p-5 border-b border-black/10 bg-white/70 backdrop-blur-xl flex-shrink-0 z-10">
                    <div className="flex items-center gap-3 overflow-hidden">
                        ...
                        {currentLesson.studyGuideUrl && (
                            <a
                                ...
                                className="inline-flex items-center gap-2 px-3 py-1.5 text-xs sm:text-sm font-semibold text-slate-700 bg-gray-500/10 rounded-full hover:bg-gray-500/20 transition-colors whitespace-nowrap"
                            >
                    ...
                    <button
                        onClick={onClose}
                        className="p-2 rounded-full text-slate-500 bg-gray-500/10 hover:bg-gray-500/20 hover:text-slate-700 transition-colors flex-shrink-0 ml-4"
                ...
                <main ref={contentRef} className="flex-grow overflow-y-auto custom-scrollbar bg-gray-100 flex flex-col items-center p-4 sm:p-8">
                            ...
                            >
                                {currentPage === 0 && objectives.length > 0 && (
                                    <motion.div
                                        ...
                                        initial="hidden"
                                        animate="visible"
                                        className="mb-8 p-5 bg-black/5 rounded-xl"
                                    >
                ...
                <footer className="flex justify-between items-center p-4 bg-white/70 backdrop-blur-xl border-t border-black/10 flex-shrink-0 z-10">
                    <div className="flex items-center gap-2">
                        <button
                            ...
                            className="p-3 rounded-full text-slate-600 bg-gray-500/10 hover:bg-gray-500/20 disabled:opacity-50 disabled:cursor-not-allowed transition-colors"
                            aria-label="Previous page"
                        >
                            <ArrowLeftIcon className="h-5 w-5" />
                        </button>
                        <div className="hidden md:flex items-center gap-2">
                            <button onClick={() => scrollContent(-200)} className="p-3 rounded-full text-slate-600 bg-gray-500/10 hover:bg-gray-500/20 transition-colors" aria-label="Scroll up">
                                <ChevronUpIcon className="h-5 w-5" />
                            </button>
                            <button onClick={() => scrollContent(200)} className="p-3 rounded-full text-slate-600 bg-gray-500/10 hover:bg-gray-500/20 transition-colors" aria-label="Scroll down">
