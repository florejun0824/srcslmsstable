...
// --- MEMOIZED LIST ITEM ---
const ContentItemRow = React.memo(({ label, value, isSelected, onToggle, index }) => (
    <li
...
const ContentGroupSection = React.memo(({ 
    groupName, 
    options, 
    selectedSet, 
    collapsedSet, 
    ...
    // Derived state
    const groupIds = useMemo(() => options.map(opt => opt.value), [options]);
    const selectedCount = groupIds.reduce((count, id) => count + (selectedSet.has(id) ? 1 : 0), 0);
    
    const isAllSelected = selectedCount === groupIds.length && groupIds.length > 0;
    const isPartiallySelected = selectedCount > 0 && !isAllSelected;
    const isCollapsed = collapsedSet.has(groupName);

    // Handlers
    const handleHeaderClick = useCallback(() => onToggleCollapse(groupName), [onToggleCollapse, groupName]);
    ...
    return (
        <div className="bg-white dark:bg-[#1a1d24] border border-slate-200 dark:border-slate-800 rounded-2xl shadow-sm mb-3 sm:mb-4 overflow-hidden transition-all">
            <header 
            ...
            >
                ...
                <div className="flex items-center gap-3 flex-grow select-none min-w-0">
                    ...
                    <div className={`w-8 h-8 rounded-full flex items-center justify-center flex-shrink-0 ${isCollapsed ? 'bg-blue-50 dark:bg-blue-900/20 text-blue-400' : 'bg-blue-100 dark:bg-blue-900/40 text-blue-600 dark:text-blue-400'}`}>
                        {isCollapsed ? <FolderIcon className="w-4 h-4" /> : <FolderOpenIcon className="w-4 h-4" />}
                    </div>
            ...
            {/* List Items - Only render if expanded for performance */}
            <div className={`transition-all duration-300 ease-in-out ${isCollapsed ? 'max-h-0 opacity-0' : 'opacity-100'}`}>
                {!isCollapsed && (
...
const ContentSelectionModal = ({ isOpen, onClose, onConfirm, title, options, currentSelection }) => {
    ...
    useEffect(() => {
        if (isOpen) {
            ...
            // Collapse all groups by default for cleaner initial view
            setCollapsedGroups(new Set(Object.keys(options))); 
        }
    ...
    const handleToggleGroup = useCallback((groupIds) => {
        ...
        setTempSelection(prevSet => {
            const allSelectedInGroup = groupIds.every(id => prevSet.has(id));
            const newSet = new Set(prevSet);
