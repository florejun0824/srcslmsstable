...
import { collection, onSnapshot, query, orderBy, doc, setDoc, deleteDoc, updateDoc, runTransaction } from 'firebase/firestore';
        ...
        return (
            <div className="absolute bottom-full mb-2 left-1 -translate-x-1 bg-neumorphic-base rounded-full shadow-neumorphic p-1 flex gap-1 z-50">
                {reactionTypes.map((rType) => (
                    ...
                    >
                        <NativeEmoji emoji={reactionIcons[rType].component({}).props.emoji} className="text-2xl" />
                    </button>
                ))}
            </div>
        );
                ...
                <div className="p-5 overflow-y-auto flex-grow">
                    <div className="flex items-center mb-4">
                        ...
                        <div>
                            ...
                            <p className="text-xs text-slate-500">{formatRelativeTime(convertTimestampToDate(announcement.createdAt))}</p>
                        </div>
                    </div>
                    ...
                    <div className="flex justify-around items-center py-1 mb-4">
                        ...
                        >
                            <button
                                className={`flex items-center justify-center py-2 px-5 rounded-full font-semibold transition-shadow hover:shadow-neumorphic-inset active:shadow-neumorphic-inset ${postReactions[currentUserId] ? 'text-blue-500' : 'text-slate-600'}`}
                                onClick={() => toggleReactionPicker(announcement.id, 'post')}
                            >
                                {postReactions[currentUserId]
                                    ? <NativeEmoji emoji={reactionIcons[postReactions[currentUserId]].component({}).props.emoji} className="text-xl" />
                                    : <FaThumbsUp className="h-5 w-5" />}
                                <span className="capitalize ml-2">{postReactions[currentUserId] || 'Like'}</span>
                            </button>
                                ...
                                <div key={comment.id} className="flex items-start space-x-3">
                                    ...
                                    <div className="flex-1 min-w-0">
                                        <div className="relative group bg-neumorphic-base p-3 rounded-xl shadow-neumorphic">
                                            ...
                                            <p className="text-sm text-slate-700 whitespace-pre-wrap mt-1 break-words">{comment.commentText}</p>
                                        </div>
                                        ...
                                        {getReplies(comment.id).map(reply => {
                                            ...
                                            return (
                                                <div key={reply.id} className="flex items-start space-x-3 mt-2 ml-10">
                                                    ...
                                                    <div className="flex-1 min-w-0">
                                                        ...
                                                        <div className="flex items-center text-xs text-slate-500 mt-1 pl-2 space-x-3">
                                                            ...
                                                            <div
                                                                className="relative flex justify-center"
                                                                onMouseEnter={() => handleReactionOptionsMouseEnter(reply.id, 'reply')}
                                                                onMouseLeave={handleReactionOptionsMouseLeave}
                                                            >
                                                                <button
                                                                    className="font-semibold hover:underline"
