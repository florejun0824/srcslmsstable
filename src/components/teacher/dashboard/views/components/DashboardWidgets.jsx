...
// 1. Mobile App Icon Style (Squircle, Deep Gradient, Fluid Press)
const mobileIconStyle = `
    relative w-full aspect-square rounded-[1.8rem] flex flex-col items-center justify-center gap-1
    shadow-[0_8px_20px_-6px_rgba(0,0,0,0.3)] hover:shadow-xl
    active:scale-90 transition-all duration-300 ease-[cubic-bezier(0.34,1.56,0.64,1)]
    border border-white/20 overflow-hidden isolate
`;

// 2. Desktop Card Style (Large, Glassy, Soft Shadows)
const desktopCardStyle = `
    relative overflow-hidden transition-all duration-300 
    h-full rounded-[2.8rem] border border-white/20 dark:border-white/5
    shadow-xl shadow-slate-200/50 dark:shadow-black/50
    hover:shadow-2xl hover:-translate-y-1
`;

// 3. Glassy Icon Box for Desktop Cards
const glassIconBox = `
    w-16 h-16 rounded-[1.2rem] flex items-center justify-center mb-5
...
const MobileClockButton = ({ onClick, style }) => {
    ...
    return (
        ...
        >
             ...
             <div className="relative z-10 flex flex-col items-center justify-center">
                 <span className="text-[17px] font-black text-white leading-none drop-shadow-md mb-1">
                     {timeString}
                 </span>
                 <span className="text-[9px] font-bold text-white/90 uppercase tracking-wide bg-white/10 px-2 py-0.5 rounded-full border border-white/10">
                     {dateString}
                 </span>
             </div>
        </motion.button>
...
const shortQuotes = [
    "Dream Big", "Be Kind", "Stay True", "Focus", "Shine On", "Create", "Inspire"
];

const MobileInspirationButton = ({ onClick, style }) => {
    const [quote, setQuote] = useState("");

    useEffect(() => {
        // Pick a random short quote on mount so it fits the tiny button
        setQuote(shortQuotes[Math.floor(Math.random() * shortQuotes.length)]);
    }, []);
...
const IconLink = ({ icon: Icon, text, onClick, style }) => (
    ...
    >
        ...
        <span className="text-[10px] font-bold text-white/95 relative z-10 tracking-widest uppercase">
            {text}
        </span>
    </motion.button>
);
                    ...
                    >
                        <Dialog.Panel className="relative w-full max-w-sm transform overflow-hidden rounded-[2.5rem] bg-[#F8F9FA] dark:bg-[#121212] p-2 shadow-2xl transition-all border border-white/50 dark:border-white/10">
                            ...
                            <button
                                type="button"
                                onClick={onClose}
                                className="absolute top-4 right-4 z-10 flex h-9 w-9 items-center justify-center rounded-full bg-white/60 dark:bg-black/40 text-slate-500 hover:bg-white dark:hover:bg-slate-700 transition-colors backdrop-blur-md shadow-sm"
                                aria-label="Close"
                            >
...
const DashboardWidgets = ({ 
    ...
    return (
        <>
            {/* --- Mobile-Only Action Bar (One UI 8.5) --- */}
            <div className="grid grid-cols-4 gap-4 sm:hidden px-2 mb-6">
                {/* 1. Live Clock Button */}
                <MobileClockButton 
                    style={accentStyle} 
                ...
                {/* 2. Live Inspiration Button (New) */}
                <MobileInspirationButton
                    style={accentStyle}
                    onClick={() => setOpenModal('inspo')}
                />
                
                {/* 3. Static Icon Buttons */}
            ...
            <div className="hidden sm:grid sm:grid-cols-2 xl:grid-cols-4 gap-6">
                ...
                {/* 3. Activities Card */}
                <div
                    className={`${desktopCardStyle} group cursor-pointer flex flex-col items-center justify-center text-center p-8`}
                    style={accentStyle}
                ...
                >
                    ...
                    <div className="relative z-10 flex flex-col items-center">
                        <div className={`${glassIconBox} group-hover:scale-110 group-hover:rotate-3 transition-transform duration-500 ease-spring`}>
                        ...
                        <p className="text-sm text-white/80 mt-2 font-bold uppercase tracking-widest px-4">
                            View Events
                        </p>
                    </div>
                </div>
            ...
            {/* --- Create Announcement Modal (Large) --- */}
                                ...
                                <Dialog.Panel className="relative w-full max-w-lg transform overflow-hidden rounded-[3rem] bg-white dark:bg-[#1E1E1E] p-8 text-left align-middle shadow-2xl transition-all border border-slate-200 dark:border-white/10">
                                    <div className="flex items-center justify-between mb-8">
                                        <Dialog.Title as="h3" className="text-2xl font-black text-slate-800 dark:text-white flex items-center gap-4 tracking-tighter">
                                            <div className="p-3.5 rounded-[1.2rem] shadow-lg shadow-black/5" style={accentStyle}>
                                                <Megaphone className="w-6 h-6 text-white" strokeWidth={3} />
                                            </div>
                                            New Post
                                        </Dialog.Title>
                                        <button type="button" onClick={() => setIsCreateModalOpen(false)} className="flex h-11 w-11 items-center justify-center rounded-full bg-slate-100 dark:bg-white/5 text-slate-500 dark:text-slate-400 hover:bg-slate-200 dark:hover:bg-white/10 transition-all shadow-sm active:scale-90">
