        ...
        <AnimatePresence>
            {isOpen && (
                <div className="fixed inset-0 z-[9999] flex items-center justify-center p-4">
                    ...
                    <motion.div initial={{ opacity: 0, scale: 0.9, y: 20 }} animate={{ opacity: 1, scale: 1, y: 0 }} exit={{ opacity: 0, scale: 0.9, y: 20 }} className="relative w-full max-w-sm bg-white/95 backdrop-blur-md rounded-[2.5rem] p-8 border border-slate-200 shadow-2xl overflow-hidden text-center">
                        <div className="w-16 h-16 rounded-full bg-red-50 flex items-center justify-center mb-5 text-red-500 mx-auto">
                            <Trash2 className="w-7 h-7" strokeWidth={2.5} />
                        </div>
                        <h3 className="text-xl font-bold text-slate-800 mb-2">Delete Post?</h3>
                        ...
                        <div className="flex flex-col w-full gap-3">
                            ...
                            <button onClick={onClose} className="w-full py-4 rounded-[1.2rem] font-bold text-slate-600 bg-slate-100 hover:bg-slate-200 active:scale-95 transition-all text-sm">Cancel</button>
                        </div>
                    </motion.div>
                </div>
            )}
...
const AnnouncementCard = forwardRef(({
    ...
    const renderTextContent = () => (
        <div className="text-[0.95rem] leading-relaxed whitespace-pre-wrap break-words tracking-wide text-slate-700 font-medium">
            <Linkify componentDecorator={(href, text, key) => (
                <a href={href} key={key} target="_blank" rel="noopener noreferrer" className="text-indigo-600 hover:underline font-bold" onClick={(e) => e.stopPropagation()}>{text}</a>
            )}>
                {isTruncated && !isExpanded ? post.content.substring(0, ANNOUNCEMENT_TRUNCATE_LENGTH) + '...' : post.content}
            </Linkify>
            {isTruncated && (
                <button onClick={(e) => { e.stopPropagation(); onToggleExpansion(post.id); }} className="block mt-2 font-bold text-xs text-indigo-600 hover:underline uppercase tracking-wider">
        ...
        <div className="mt-auto pt-6">
                    ...
                    <div className="flex items-center gap-2 cursor-pointer group select-none" onClick={(e) => { e.stopPropagation(); onViewReactions(postReactions, usersMap); }}>
                        <div className="flex items-center -space-x-2">
                            {reactionStats.sortedTypes.slice(0, 3).map((type, index) => (
                                <div key={type} style={{ zIndex: 30 - index }} className="relative w-5 h-5 rounded-full bg-white ring-2 ring-white">
                                    <img src={reactionIconsHomeView[type]?.static} className="w-full h-full object-contain" alt={type} />
                                </div>
                            ))}
                        </div>
                        <span className="text-xs font-bold text-slate-500 group-hover:text-indigo-500 transition-colors">{reactionStats.total}</span>
                    </div>
                ) : <div className="text-xs font-bold text-slate-400">Be the first to react</div>}
                <button className="text-xs font-bold text-slate-500 hover:text-indigo-500 transition-colors" onClick={(e) => { e.stopPropagation(); onViewComments(post); }}>
            ...
            <div className="grid grid-cols-2 gap-3">
                <div className="relative" onMouseEnter={handleReactionOptionsMouseEnter} onMouseLeave={handleReactionOptionsMouseLeave}>
                    <button 
                        className={`flex items-center justify-center gap-2 py-3 rounded-[1.2rem] w-full transition-all active:scale-95 ${currentUserReaction ? 'bg-indigo-50 text-indigo-600' : 'bg-slate-50 text-slate-600 hover:bg-slate-100'}`} 
                    ...
                    <AnimatePresence>
                        {isReactionOptionsVisible && (
                            <motion.div initial={{ opacity: 0, y: 10, scale: 0.9 }} animate={{ opacity: 1, y: 0, scale: 1 }} exit={{ opacity: 0, scale: 0.9 }} className="absolute bottom-full mb-2 left-0 p-1.5 flex gap-1 rounded-full bg-white shadow-xl border border-slate-100 z-50">
                ...
                <button 
                    className="flex items-center justify-center gap-2 py-3 rounded-[1.2rem] w-full bg-slate-50 text-slate-600 hover:bg-slate-100 active:scale-95 transition-all" 
    ...
    return (
        <>
            ...
            <div ref={ref} className="group relative w-full bg-white rounded-[2.5rem] shadow-xl border border-slate-200 transition-all hover:shadow-2xl overflow-hidden">
                
                {/* --- PINNED INDICATOR --- */}
                {post.isPinned && (
                    <div className="absolute top-0 right-0 z-30 p-4">
                         <div className="bg-blue-500/10 backdrop-blur-md border border-blue-500/20 text-blue-600 p-2 rounded-full">
                    ...
                    <div className="flex flex-col lg:flex-row h-full">
                        ...
                        <div className="flex-1 p-6 sm:p-8 flex flex-col min-h-[300px] relative z-10 order-2 lg:order-1">
                            ...
                            <div className="flex items-center gap-4 mb-6">
                                <div className="w-12 h-12 rounded-2xl overflow-hidden bg-slate-100"><UserInitialsAvatar user={authorProfile} size="full" /></div>
                                <div>
                                    <h3 className="font-bold text-slate-900 text-base leading-tight">{post.teacherName}</h3>
                                    <p className="text-xs font-semibold text-slate-400 uppercase tracking-wide mt-0.5">{post.createdAt ? new Date(post.createdAt.toDate()).toLocaleDateString([], { month: 'short', day: 'numeric', hour: '2-digit', minute:'2-digit' }) : 'Just now'}</p>
                                </div>
                                {/* Context Menu */}
                                {canModify && (
                                    <div className="ml-auto flex gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                                        <button onClick={(e) => { e.stopPropagation(); onStartEdit(post); }} className="p-2 hover:bg-slate-100 rounded-full text-slate-500"><Pencil className="w-4 h-4" /></button>
                                        <button onClick={(e) => { e.stopPropagation(); setShowDeleteConfirm(true); }} className="p-2 hover:bg-red-50 rounded-full text-red-500"><Trash2 className="w-4 h-4" /></button>
                                    </div>
                                )}
                            </div>
                        ...
                        <div className="lg:w-[40%] min-h-[250px] lg:min-h-[400px] relative order-1 lg:order-2 overflow-hidden bg-slate-100 cursor-pointer" onClick={() => window.open(post.photoURL, '_blank')}>
                    ...
                    <div className="p-6 sm:p-8 relative">
                         ...
                         <div className="flex items-center justify-between mb-6">
                            <div className="flex items-center gap-4">
                                <div className="w-12 h-12 rounded-2xl overflow-hidden bg-slate-100"><UserInitialsAvatar user={authorProfile} size="full" /></div>
                                <div>
                                    <h3 className="font-bold text-slate-900 text-lg">{post.teacherName}</h3>
                        ...
                        {isEditing ? (
                            <div className="bg-slate-50 p-4 rounded-3xl border border-slate-200">
                                <textarea className="w-full bg-transparent resize-none focus:outline-none text-slate-800 text-sm font-medium" rows="4" value={editingText} onChange={(e) => onTextChange(e.target.value)} autoFocus />
                            ...
                            <div className="bg-slate-50/50 p-6 rounded-[2rem] border border-slate-100 mb-2">
                                {renderTextContent()}
                            </div>
                        )}
