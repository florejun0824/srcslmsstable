...
import { Pencil, Trash2, Pin, MessageCircle, ThumbsUp, AlertTriangle, X } from 'lucide-react';
...
// --- HELPER: MONET CARD STYLES (UPDATED OPACITY) ---
const getMonetCardStyle = (activeOverlay) => {
    ...
    // UPDATED: Background opacity increased to /90 or /95 for less transparency
    switch (activeOverlay) {
        case 'christmas': return { ...base, card: `${base.card} bg-slate-900/90 border-emerald-500/20 shadow-emerald-900/20` };
        case 'valentines': return { ...base, card: `${base.card} bg-[#2c0b0e]/90 border-rose-500/20 shadow-rose-900/20` };
        case 'graduation': return { ...base, card: `${base.card} bg-[#1a1400]/90 border-amber-500/20 shadow-amber-900/20` };
        case 'rainy': return { ...base, card: `${base.card} bg-[#061816]/90 border-teal-500/20 shadow-teal-900/20` };
        case 'cyberpunk': return { ...base, card: `${base.card} bg-[#180a20]/90 border-fuchsia-500/20 shadow-fuchsia-900/20` };
        case 'spring': return { ...base, card: `${base.card} bg-[#1f0f15]/90 border-pink-500/20 shadow-pink-900/20` };
        case 'space': return { ...base, card: `${base.card} bg-[#020617]/90 border-indigo-500/20 shadow-indigo-900/20` };
        default: return null;
    }
};

// --- ONE UI DELETE DIALOG COMPONENT ---
            ...
            {isOpen && (
                <div className="fixed inset-0 z-[9999] flex items-center justify-center p-4">
                    ...
                    <motion.div
                        ...
                        className="relative w-full max-w-sm bg-white/95 dark:bg-[#1C1C1E]/95 backdrop-blur-3xl rounded-[2.5rem] p-8 border border-white/20 shadow-[0_40px_80px_-15px_rgba(0,0,0,0.3)] dark:shadow-black/50 overflow-hidden text-center"
                    >
                        ...
                        <div className="relative z-10 flex flex-col items-center">
                            ...
                            <h3 className="text-xl font-bold text-slate-900 dark:text-white mb-2 tracking-tight">Delete Post?</h3>
                            <p className="text-sm font-medium text-slate-500 dark:text-slate-400 mb-8 leading-relaxed max-w-[200px]">
                            ...
                            <div className="flex flex-col w-full gap-3">
                                <button 
                                    onClick={onConfirm}
                                    className="w-full py-4 rounded-[1.2rem] font-bold text-white bg-red-500 hover:bg-red-600 shadow-lg shadow-red-500/30 active:scale-95 transition-all text-sm tracking-wide"
                                >
                                    Yes, Delete
                                </button>
                                <button 
                                    onClick={onClose}
                                    className="w-full py-4 rounded-[1.2rem] font-bold text-slate-600 dark:text-slate-300 bg-slate-100 dark:bg-white/5 hover:bg-slate-200 dark:hover:bg-white/10 active:scale-95 transition-all text-sm tracking-wide"
                                >
                                    Cancel
                                </button>
                            </div>
    ...
    return (
        <>
            ...
            <motion.div
                ...
                // UPDATED: Increased opacity for default card state (/95)
                className={`rounded-[2.5rem] p-7 relative group transition-all duration-300 font-sans backdrop-blur-3xl
                    ${monet ? monet.card : 'bg-white/95 dark:bg-[#121212]/95 border border-white/50 dark:border-white/5 shadow-xl hover:shadow-2xl'}`}
            >
                {/* PINNED BADGE */}
                {post.isPinned && (
