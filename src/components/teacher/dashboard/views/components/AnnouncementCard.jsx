        ...
        <AnimatePresence>
            {isOpen && (
                <div className="fixed inset-0 z-[9999] flex items-center justify-center p-4">
                    ...
                    <motion.div initial={{ opacity: 0, scale: 0.9, y: 20 }} animate={{ opacity: 1, scale: 1, y: 0 }} exit={{ opacity: 0, scale: 0.9, y: 20 }} className="relative w-full max-w-sm bg-white/95 dark:bg-[#1e1e2d] backdrop-blur-md rounded-[2.5rem] p-8 border border-slate-200 dark:border-white/10 shadow-2xl overflow-hidden text-center">
                        ...
                        <h3 className="text-xl font-bold text-slate-800 dark:text-slate-100 mb-2">Delete Post?</h3>
...
const AnnouncementCard = forwardRef(({
    ...
    const renderTextContent = () => (
        <div className="text-[0.95rem] leading-relaxed whitespace-pre-wrap break-words tracking-wide text-slate-700 dark:text-slate-300 font-medium">
        ...
        <div className="mt-auto pt-6">
            ...
            <div className="grid grid-cols-2 gap-3">
                <div className="relative" onMouseEnter={handleReactionOptionsMouseEnter} onMouseLeave={handleReactionOptionsMouseLeave}>
                    <button
                        className={`flex items-center justify-center gap-2 py-3 rounded-[1.2rem] w-full transition-all active:scale-95 ${currentUserReaction ? 'bg-indigo-50 dark:bg-indigo-500/20 text-indigo-600 dark:text-indigo-400' : 'bg-slate-50 dark:bg-white/5 text-slate-600 dark:text-slate-400 hover:bg-slate-100 dark:hover:bg-white/10'}`}
                    ...
                    <AnimatePresence>
                        {isReactionOptionsVisible && (
                            <motion.div initial={{ opacity: 0, y: 10, scale: 0.9 }} animate={{ opacity: 1, y: 0, scale: 1 }} exit={{ opacity: 0, scale: 0.9 }} className="absolute bottom-full mb-2 left-0 p-1.5 flex gap-1 rounded-full bg-white dark:bg-[#1e1e2d] shadow-xl border border-slate-100 dark:border-white/10 z-50">
    ...
    return (
        <>
            ...
            <div ref={ref} className="group relative w-full bg-white dark:bg-white/5 backdrop-blur-xl rounded-[2.5rem] shadow-xl border border-slate-200 dark:border-white/10 transition-all hover:shadow-2xl overflow-hidden">
                ...
                {hasImage && !isEditing ? (
                    ...
                    <div className="flex flex-col lg:flex-row h-full">
                        ...
                        <div className="flex-1 p-6 sm:p-8 flex flex-col min-h-[300px] relative z-10 order-2 lg:order-1">
                            ...
                            <div className="flex items-center gap-4 mb-6">
                                <div className="w-12 h-12 rounded-2xl overflow-hidden bg-slate-100 dark:bg-white/10"><UserInitialsAvatar user={authorProfile} size="full" /></div>
                                <div>
                                ...
                                {canModify && (
                                    <div className="ml-auto flex gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                                        <button onClick={(e) => { e.stopPropagation(); onStartEdit(post); }} className="p-2 hover:bg-slate-100 dark:hover:bg-white/10 rounded-full text-slate-500 dark:text-slate-400"><Pencil className="w-4 h-4" /></button>
                                        <button onClick={(e) => { e.stopPropagation(); setShowDeleteConfirm(true); }} className="p-2 hover:bg-red-50 dark:hover:bg-red-500/10 rounded-full text-red-500"><Trash2 className="w-4 h-4" /></button>
                                    </div>
                                )}
                            </div>
                    ...
                    <div className="p-6 sm:p-8 relative">
                        ...
                        <div className="flex items-center justify-between mb-6">
                            <div className="flex items-center gap-4">
                                ...
                                <div>
                                    ...
                                    <p className="text-xs font-semibold text-slate-400 dark:text-slate-500 uppercase tracking-wide">{post.createdAt ? new Date(post.createdAt.toDate()).toLocaleDateString() : ''}</p>
                                </div>
                            </div>
                            {canModify && (
                        ...
                        {isEditing ? (
                            ...
                            <div className="bg-slate-50/50 dark:bg-white/5 p-6 rounded-[2rem] border border-slate-100 dark:border-white/10 mb-2">
                                {renderTextContent()}
                            </div>
                        )}
