...
const ActivityFeed = ({ userProfile, teacherAnnouncements, showToast }) => {
    ...
    const openAnnouncementModal = useCallback((announcement) => { setSelectedAnnouncement(announcement); setIsAnnouncementModalOpen(true); }, []);
    const closeAnnouncementModal = useCallback(() => { setIsAnnouncementModalOpen(false); setSelectedAnnouncement(null); }, []);
    const openReactionsBreakdownModal = useCallback((reactions, users) => { setReactionsForBreakdownModal({ reactions, users }); setIsReactionsBreakdownModalOpen(true); }, []);
    ...
    return (
        ...
        <div className="space-y-4 w-full relative z-10 pb-8">
            <div className="flex items-center gap-4 px-2">
                ...
                <div>
                    <h2 className={`text-2xl font-bold tracking-tight ${monetStyles ? monetStyles.textColor : 'text-slate-900 dark:text-white'}`}>Announcements</h2>
                </div>
            </div>
            ...
            <Suspense fallback={null}>
                {isAnnouncementModalOpen && <AnnouncementModal isOpen={isAnnouncementModalOpen} onClose={closeAnnouncementModal} announcement={selectedAnnouncement} userProfile={userProfile} db={db} postReactions={selectedAnnouncement ? postReactions[selectedAnnouncement.id] : {}} onToggleReaction={handleTogglePostReaction} usersMap={usersMap} />}
                {isReactionsBreakdownModalOpen && <ReactionsBreakdownModal isOpen={isReactionsBreakdownModalOpen} onClose={closeReactionsBreakdownModal} reactionsData={reactionsForBreakdownModal?.reactions} usersMap={reactionsForBreakdownModal?.users} />}
            </Suspense>
        </div>
    );
};
