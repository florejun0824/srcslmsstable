...
const MATERIAL_STYLES = {
    bgScaffold: "courses-system-font bg-white/60 m-0 sm:m-4 rounded-[24px] sm:rounded-[32px] border border-zinc-200/50 shadow-xl flex-1 flex flex-col min-h-[calc(100vh-6rem)] relative selection:bg-indigo-200 selection:text-indigo-900 overflow-visible",
    bgSurface: "bg-white shadow-sm border border-zinc-200/50 rounded-[24px] sm:rounded-[32px]",
    navPill: "bg-white border border-zinc-200/50 rounded-full px-4 py-2 flex items-center gap-2 shadow-sm",
    textOnSurface: "text-zinc-900 tracking-tight",
    textVariant: "text-zinc-500 font-medium",
    ...
    btnTonal: "flex items-center justify-center gap-2 px-4 py-2 md:px-5 md:py-2.5 rounded-full font-bold text-xs md:text-sm transition-all duration-200 active:scale-95 bg-indigo-100 text-indigo-900 hover:bg-indigo-200 flex-shrink-0",
    btnIcon: "p-2.5 rounded-full hover:bg-zinc-200/50 transition-colors text-zinc-600 active:scale-95 flex-shrink-0",
    searchBar: "w-full pl-10 pr-4 py-2.5 rounded-full bg-zinc-100/80 border border-zinc-200/50 text-zinc-900 placeholder-zinc-500 focus:outline-none focus:ring-2 focus:ring-indigo-500/50 transition-all font-medium text-sm"
};
...
const ContentScopeSwitcher = memo(({ activeGroup, onSwitch }) => {
    ...
    return (
        <div className="flex p-1 bg-zinc-200/50 border border-zinc-200/50 rounded-full h-10 md:h-12 items-center">
            <button onClick={() => onSwitch('learner')} className={`px-4 md:px-5 h-full rounded-full text-xs md:text-sm font-bold transition-all duration-300 flex items-center justify-center gap-2 flex-1 md:flex-none ${isLearner ? 'bg-emerald-600 text-white shadow-sm' : 'text-zinc-600 hover:bg-zinc-300/30'}`}>
                <LearnerIcon className="w-4 h-4" /> Learner
            </button>
            <button onClick={() => onSwitch('teacher')} className={`px-4 md:px-5 h-full rounded-full text-xs md:text-sm font-bold transition-all duration-300 flex items-center justify-center gap-2 flex-1 md:flex-none ${!isLearner ? 'bg-indigo-600 text-white shadow-sm' : 'text-zinc-600 hover:bg-zinc-300/30'}`}>
...
const Breadcrumbs = ({ contentGroup, categoryName, subjectTitle, unitTitle, subjectId }) => (
    <nav className="inline-flex items-center gap-1.5 md:gap-2 max-w-full overflow-x-auto hide-scrollbar pb-2 md:pb-0">
        <Link to="/dashboard/courses" className="p-2 rounded-full bg-white border border-zinc-200/50 hover:bg-zinc-50 transition-colors flex-shrink-0">
            <HomeIcon className="w-4 h-4 text-zinc-600" />
        </Link>
        <ChevronRightIcon className="w-3.5 h-3.5 text-zinc-400 flex-shrink-0" />

        <Link to={`/dashboard/courses/${contentGroup}`} className="px-3 md:px-4 py-1.5 md:py-2 rounded-full bg-white border border-zinc-200/50 hover:bg-zinc-50 text-xs md:text-sm font-bold text-zinc-700 capitalize transition-colors whitespace-nowrap flex-shrink-0">
            {contentGroup}
        </Link>
        <ChevronRightIcon className="w-3.5 h-3.5 text-zinc-400 flex-shrink-0" />

        <Link to={`/dashboard/courses/${contentGroup}/${categoryName}`} className="px-3 md:px-4 py-1.5 md:py-2 rounded-full bg-white border border-zinc-200/50 hover:bg-zinc-50 text-xs md:text-sm font-bold text-zinc-700 whitespace-nowrap max-w-[120px] md:max-w-[150px] truncate transition-colors flex-shrink-0">
        ...
        {subjectTitle && (
            <>
                <ChevronRightIcon className="w-3.5 h-3.5 text-zinc-400 flex-shrink-0" />
                {unitTitle && subjectId ? (
                    <Link to={`/dashboard/courses/${contentGroup}/${categoryName}/${subjectId}`} className="px-3 md:px-4 py-1.5 md:py-2 rounded-full text-xs md:text-sm font-bold bg-indigo-50 text-indigo-900 hover:bg-indigo-100 transition-colors whitespace-nowrap max-w-[120px] md:max-w-[200px] truncate flex-shrink-0">
                        {subjectTitle}
                    </Link>
                ) : (
                    <span className={`px-3 md:px-4 py-1.5 md:py-2 rounded-full text-xs md:text-sm font-bold ${!unitTitle ? 'bg-indigo-50 text-indigo-900' : 'bg-white border border-zinc-200/50 text-zinc-700'} whitespace-nowrap max-w-[120px] md:max-w-[200px] truncate flex-shrink-0`}>
                        {subjectTitle}
                    </span>
                )}
            </>
        ...
        {unitTitle && (
            <>
                <ChevronRightIcon className="w-3.5 h-3.5 text-zinc-400 flex-shrink-0" />
                <span className="px-3 md:px-4 py-1.5 md:py-2 rounded-full text-xs md:text-sm font-bold bg-rose-50 text-rose-900 whitespace-nowrap max-w-[120px] md:max-w-[200px] truncate flex-shrink-0">
                    {unitTitle}
                </span>
            </>
        )}
...
const SubjectDetail = memo((props) => {
    ...
    return (
        <div className={`flex flex-col h-full min-h-[calc(100vh-6rem)] ${MATERIAL_STYLES.bgScaffold}`}>
            ...
            <header className="sticky top-0 z-50 flex-none px-4 md:px-6 py-3 md:py-4 flex flex-col md:flex-row items-start md:items-center justify-between gap-3 border-b border-zinc-200/50 bg-white rounded-t-[24px] sm:rounded-t-[32px]">
                ...
                <div className="flex items-center gap-2 w-full md:w-auto justify-end">
                    {!activeUnit && (
                        <div className="hidden md:flex items-center bg-zinc-50 rounded-full p-1 border border-zinc-200/50">
                            <button onClick={() => handleOpenEditSubject(activeSubject)} className="p-2 rounded-full hover:bg-zinc-200/50 transition-colors text-zinc-600" title="Edit"><PencilSquareIcon className="w-4 h-4" /></button>
                            <button onClick={() => handleInitiateDelete('subject', activeSubject.id, activeSubject.title)} className="p-2 rounded-full hover:bg-red-50 transition-colors text-zinc-600 hover:text-red-600" title="Delete"><TrashIcon className="w-4 h-4" /></button>
                        </div>
                    )}
            ...
            {showLessonPicker && activeUnitForPicker && (
                <div className="fixed inset-0 z-[5000] flex items-end sm:items-center justify-center sm:p-6 pointer-events-auto">
                    ...
                    <div className={`relative w-full sm:max-w-xl bg-white rounded-t-[32px] sm:rounded-[36px] shadow-2xl flex flex-col h-[85vh] sm:max-h-[80vh] overflow-hidden animate-spring-up border border-zinc-200/50 z-10`}>

                        <div className="w-full flex justify-center pt-4 pb-2 sm:hidden">
                            <div className="w-12 h-1.5 rounded-full bg-zinc-300"></div>
                        </div>

                        <div className="flex-none px-6 sm:px-8 pt-4 sm:pt-8 pb-4 flex items-center justify-between border-b border-zinc-100">
                            <div>
                                <h2 className="text-xl sm:text-2xl font-bold text-zinc-900 tracking-tight">
                                    Select Lessons
                                </h2>
                                <p className="text-sm font-medium text-zinc-500 mt-1 truncate max-w-[250px] sm:max-w-sm">
                                    {activeUnitForPicker.title}
                                </p>
                            </div>
                            <button
                                onClick={() => setShowLessonPicker(false)}
                                className="p-2.5 rounded-full bg-zinc-100 hover:bg-zinc-200 transition-colors text-zinc-600"
                            >
                                    ...
                                    return (
                                        <div className="h-full flex flex-col items-center justify-center p-8 text-center">
                                            <div className="w-16 h-16 rounded-full bg-zinc-100 flex items-center justify-center mb-4">
                                                <BookOpenIcon className="w-8 h-8 text-zinc-400" />
                                            </div>
                                            <p className="text-base font-bold text-zinc-900">No lessons found</p>
                                            <p className="text-sm font-medium text-zinc-500 mt-1">Add lessons to this unit first.</p>
                                        </div>
                                    );
                                }
                        ...
                        <div className="flex-none p-4 sm:p-6 border-t border-zinc-200/50 bg-white">
                            <div className="flex flex-col sm:flex-row items-center justify-between gap-4">
                                <p className="text-sm font-bold text-zinc-500 w-full sm:w-auto text-center sm:text-left">
                                    <span className="text-indigo-600 text-lg mr-1">{selectedLessons.size}</span>
                                    Selected
                                </p>
                                ...
                                <div className="flex gap-3 w-full sm:w-auto">
                                    <button
                                        onClick={() => setShowLessonPicker(false)}
                                        className="flex-1 sm:flex-none px-5 py-3 rounded-full text-sm font-bold text-zinc-700 bg-zinc-100 hover:bg-zinc-200 transition-colors"
                                    >
                                        Cancel
                                    </button>
                                    <button
...
const SubjectList = memo((props) => {
    ...
    return (
        <div className={`flex flex-col h-full min-h-[calc(100vh-6rem)] ${MATERIAL_STYLES.bgScaffold}`}>
            ...
            <header className="sticky top-0 z-50 flex-none px-4 md:px-6 py-4 md:py-5 flex flex-col md:flex-row items-start md:items-center justify-between gap-4 border-b border-transparent bg-zinc-50 rounded-t-[24px] sm:rounded-t-[32px]">
                                    ...
                                    <div className="flex items-center justify-between mt-auto pt-3 border-t border-black/[0.04] relative z-10">
                                        ...
                                        <div className="flex items-center gap-1" onClick={(e) => { e.preventDefault(); e.stopPropagation(); }}>
                                            <button
                                                ...
                                                className="p-2 rounded-xl bg-white/60 hover:bg-white text-zinc-500 hover:text-zinc-700 transition-all shadow-sm"
                                                title="Edit Subject"
                                            >
                ...
                {loading ? <SkeletonGrid /> : (
                    <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-3 md:gap-5 animate-spring-up">
                        {categoriesToShow.map((cat, idx) => {
                            ...
                            const cardThemes = [
                                { bg: 'bg-sky-50/80', border: 'border-sky-200/50', hoverBorder: 'hover:border-sky-300', iconBg: 'bg-sky-100', iconText: 'text-sky-700', badge: 'bg-sky-100 text-sky-600', accent: 'bg-sky-400' },
                                { bg: 'bg-rose-50/80', border: 'border-rose-200/50', hoverBorder: 'hover:border-rose-300', iconBg: 'bg-rose-100', iconText: 'text-rose-700', badge: 'bg-rose-100 text-rose-600', accent: 'bg-rose-400' },
                                { bg: 'bg-sky-50/80', border: 'border-sky-200/50', hoverBorder: 'hover:border-sky-300', iconBg: 'bg-sky-100', iconText: 'text-sky-700', badge: 'bg-sky-100 text-sky-600', accent: 'bg-sky-400' },
                                { bg: 'bg-emerald-50/80', border: 'border-emerald-200/50', hoverBorder: 'hover:border-emerald-300', iconBg: 'bg-emerald-100', iconText: 'text-emerald-700', badge: 'bg-emerald-100 text-emerald-600', accent: 'bg-emerald-400' },
                                { bg: 'bg-amber-50/80', border: 'border-amber-200/50', hoverBorder: 'hover:border-amber-300', iconBg: 'bg-amber-100', iconText: 'text-amber-700', badge: 'bg-amber-100 text-amber-600', accent: 'bg-amber-400' },
                            ];
                            ...
                            return (
                                ...
                                >
                                    ...
                                    <div className="absolute top-2.5 right-2.5 opacity-0 group-hover:opacity-100 transition-opacity flex flex-col gap-1.5 z-10">
                                        <button onClick={(e) => { e.preventDefault(); handleEditCategory(cat) }} className="p-1.5 md:p-2 rounded-full bg-white/80 hover:bg-white transition-colors shadow-sm backdrop-blur-sm"><PencilSquareIcon className="w-3.5 h-3.5 md:w-4 md:h-4 text-zinc-600" /></button>
                                        <button onClick={(e) => { e.preventDefault(); handleInitiateDelete('category', cat.id, cat.name) }} className="p-1.5 md:p-2 rounded-full bg-red-50/80 hover:bg-red-100 transition-colors shadow-sm backdrop-blur-sm"><TrashIcon className="w-3.5 h-3.5 md:w-4 md:h-4 text-red-600" /></button>
                                    </div>
...
const ContentGroupSelector = memo((props) => {
    ...
    const SelectionCard = ({ to, title, subtitle, themeClass, icon: Icon }) => (
        <Link to={to} className={`group relative overflow-hidden rounded-[32px] md:rounded-[40px] p-6 md:p-10 h-[280px] md:h-[340px] flex flex-col justify-between transition-all duration-300 hover:scale-[1.01] hover:shadow-xl shadow-md border border-black/5 ${themeClass}`}>
            ...
            <div className="relative z-10">
                <div className={`w-16 h-16 md:w-20 md:h-20 rounded-[20px] md:rounded-[28px] flex items-center justify-center mb-6 md:mb-8 bg-white/20 shadow-sm border border-white/20`}>
    ...
    return (
        <div className={`min-h-[calc(100vh-6rem)] flex flex-col items-center justify-center p-4 md:p-6 ${MATERIAL_STYLES.bgScaffold}`}>
            <div className="mb-8 md:mb-12 text-center animate-spring-up">
                <div className="w-20 h-20 md:w-24 md:h-24 mx-auto mb-5 md:mb-6 rounded-[28px] md:rounded-[32px] bg-white shadow-md flex items-center justify-center p-2 border border-zinc-200/50">
