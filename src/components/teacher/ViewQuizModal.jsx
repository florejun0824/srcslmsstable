...
export default function ViewQuizModal({ isOpen, onClose, onComplete, quiz, userProfile, classId, isTeacherView = false }) {
    ...
    useEffect(() => {
        let listener;
        if (isOpen && Capacitor.isNativePlatform() && score === null && !isLocked && !isTeacherView && (quiz?.settings?.lockOnLeave ?? false)) {
            listener = App.addListener('appStateChange', ({ isActive }) => { if (!isActive) { issueWarning(); } });
        }
        return () => { listener?.remove(); };
    }, [isOpen, score, isLocked, issueWarning, isTeacherView, quiz?.settings?.lockOnLeave]);

    useEffect(() => {
        const handleFocusLoss = () => { if (!isOpen || hasSubmitted.current || isLocked || !classId || score !== null || showReview || isTeacherView || !(quiz?.settings?.lockOnLeave ?? false)) { return; } issueWarning(); };
        window.addEventListener('blur', handleFocusLoss);
        return () => window.removeEventListener('blur', handleFocusLoss);
    }, [isOpen, isLocked, score, classId, issueWarning, showReview, isTeacherView, quiz?.settings?.lockOnLeave]);
