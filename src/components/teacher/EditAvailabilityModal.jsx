...
// --- DESIGN SYSTEM CONSTANTS (Updated for Mobile Compactness) ---
const glassPanel = "bg-white/60 dark:bg-[#1a1d24]/60 backdrop-blur-xl border border-white/40 dark:border-white/5 shadow-lg rounded-xl sm:rounded-2xl transition-all";

// Inputs: Smaller text and padding on mobile
const glassInput = "w-full bg-white/50 dark:bg-black/20 border border-slate-200/60 dark:border-white/10 rounded-lg sm:rounded-xl px-3 py-2 sm:px-4 sm:py-3 text-xs sm:text-sm text-slate-800 dark:text-slate-100 placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-blue-500/50 transition-all";

// Buttons: Compact on mobile
const primaryBtn = "px-4 py-2 sm:px-6 sm:py-3 rounded-lg sm:rounded-xl font-bold text-xs sm:text-sm text-white shadow-lg shadow-blue-500/30 bg-gradient-to-b from-blue-500 to-blue-600 hover:from-blue-400 hover:to-blue-500 border border-blue-400/20 active:scale-[0.98] transition-all duration-200 disabled:opacity-50 disabled:cursor-not-allowed whitespace-nowrap";
const secondaryBtn = "px-4 py-2 sm:px-6 sm:py-3 rounded-lg sm:rounded-xl font-bold text-xs sm:text-sm text-slate-600 dark:text-slate-300 bg-white/50 dark:bg-white/5 hover:bg-white/80 dark:hover:bg-white/10 border border-white/20 dark:border-white/5 active:scale-[0.98] transition-all duration-200 disabled:opacity-50 whitespace-nowrap";
const deleteBtn = "px-4 py-2 sm:px-6 sm:py-3 rounded-lg sm:rounded-xl font-bold text-xs sm:text-sm text-red-600 dark:text-red-400 bg-red-50/50 dark:bg-red-900/10 hover:bg-red-100 dark:hover:bg-red-900/20 border border-red-200/50 dark:border-red-500/20 active:scale-[0.98] transition-all duration-200 disabled:opacity-50 whitespace-nowrap";

// --- (Refined Checkbox - Responsive Size) ---
    ...
    return (
        <div className="relative w-4 h-4 sm:w-5 sm:h-5 flex-shrink-0">
            ...
            <span className={`w-full h-full rounded-md border flex items-center justify-center transition-all duration-200 ${
                ...
                <CheckIcon className={`w-3 h-3 sm:w-3.5 sm:h-3.5 text-white transition-transform duration-200 ${checked ? 'scale-100' : 'scale-0'}`} />
            </span>
        </div>
    );
});

// --- (Refined ToggleSwitch - Responsive Size) ---
const ToggleSwitch = ({ label, enabled, onChange, disabled = false }) => (
    <label className={`flex items-center justify-between group ${disabled ? 'cursor-not-allowed opacity-60' : 'cursor-pointer'}`}>
        <span className="font-medium text-xs sm:text-sm text-slate-700 dark:text-slate-200 group-hover:text-slate-900 dark:group-hover:text-white transition-colors mr-2">{label}</span>
        <div className="relative">
            <input type="checkbox" className="sr-only" checked={enabled} onChange={onChange} disabled={disabled} />
            {/* Mobile: w-10 h-6, Desktop: w-12 h-7 */}
            <div className={`block w-10 h-6 sm:w-12 sm:h-7 rounded-full transition-all duration-300 ${
                enabled 
                ? 'bg-blue-500 shadow-[0_0_10px_rgba(59,130,246,0.5)]' 
                : 'bg-slate-200 dark:bg-slate-700 inner-shadow'
            }`}></div>
            {/* Mobile: w-4 h-4, Desktop: w-5 h-5 */}
            <div className={`absolute left-1 top-1 bg-white w-4 h-4 sm:w-5 sm:h-5 rounded-full shadow-sm transition-transform duration-300 ${enabled ? 'translate-x-4 sm:translate-x-5' : ''}`}></div>
        </div>
    </label>
);
...
// --- (Refined Empty State - Responsive Text) ---
const EmptyState = ({ icon: Icon, text, subtext }) => (
    <div className="text-center p-6 sm:p-8 border-2 border-dashed border-slate-200 dark:border-white/10 rounded-2xl h-full flex flex-col justify-center items-center opacity-70">
        <div className="w-12 h-12 sm:w-16 sm:h-16 rounded-full bg-slate-100 dark:bg-white/5 flex items-center justify-center mb-4">
            <Icon className="h-6 w-6 sm:h-8 sm:w-8 text-slate-400 dark:text-slate-500" />
        </div>
        <p className="text-sm sm:text-lg font-bold text-slate-700 dark:text-slate-200">{text}</p>
        <p className="mt-1 text-xs sm:text-sm text-slate-500 dark:text-slate-400">{subtext}</p>
    </div>
);
...
const EditAvailabilityModal = ({ isOpen, onClose, post, classId, onUpdate, classData }) => {
    ...
    return (
        ...
        >
            <div className="p-4 sm:p-8 bg-white/90 dark:bg-[#121212]/95 h-full sm:h-[90vh] max-h-full sm:max-h-[90vh] flex flex-col mx-auto w-full rounded-xl sm:rounded-[2rem] shadow-2xl border border-white/20 dark:border-white/5">
                
                {/* Header */}
                <header className="mb-4 sm:mb-6 flex-shrink-0 border-b border-slate-200/60 dark:border-white/5 pb-3 sm:pb-4">
                ...
                <nav className="lg:hidden flex-shrink-0 flex items-center gap-1.5 p-1 bg-slate-100 dark:bg-white/5 rounded-lg overflow-x-auto mb-4 scrollbar-hide">
                    {mobileTabs.map(tab => (
                        <button
                            key={tab.id}
                ...
                <main className="flex-1 grid grid-cols-1 lg:grid-cols-3 gap-6 lg:gap-8 min-h-0">
                    ...
                    `}>
                        ...
                        <div className="flex-1 flex flex-col space-y-4 sm:space-y-6 overflow-y-auto custom-scrollbar min-h-0 pr-1 sm:pr-2">
                            <div className={glassPanel + " p-4 sm:p-6 space-y-4 sm:space-y-5"}> 
                                <div>
                                ...
                                <div>
                                    <label className="block text-[10px] sm:text-xs font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wide mb-1.5 ml-1">Instructions (Optional)</label>
                                    <textarea
                                        value={postContent}
                                ...
                                <div>
                                    ...
                                    <div className="flex flex-col sm:flex-row gap-2 sm:gap-3">
                                        ...
                                        <input
                                            ...
                                            className={`${glassInput} w-full sm:w-1/3 text-center`}
                                        />
                                    </div>
                                </div>
                    ...
                    `}>
                        <div className="flex justify-between items-end mb-1 sm:mb-2">
                            <h2 className="text-[10px] sm:text-xs font-bold text-slate-400 dark:text-slate-500 uppercase tracking-widest">
                                Recipients
                            </h2>
                            <span className="text-[10px] sm:text-xs font-bold text-blue-600 dark:text-blue-400 bg-blue-50 dark:bg-blue-900/20 px-2 py-0.5 sm:py-1 rounded-md">
                        ...
                        <div className="relative flex-shrink-0">
                            <input
                                ...
                                className={`${glassInput} pl-9 sm:pl-10`}
                            />
                        ...
                        <div className={`${glassPanel} flex-1 overflow-hidden flex flex-col min-h-0`}>
                            <header 
                                ...
                                className="flex items-center gap-3 p-3 sm:p-4 border-b border-slate-200/50 dark:border-white/5 cursor-pointer hover:bg-slate-50/50 dark:hover:bg-white/5 transition-colors flex-shrink-0"
                            >
                                <NeumorphicCheckbox
                                ...
                                />
                                <label className="font-bold text-xs sm:text-sm text-slate-700 dark:text-slate-200 flex-grow cursor-pointer select-none">
                                    Select All
                                </label>
                                <span className="text-[10px] sm:text-xs font-bold text-slate-400 dark:text-slate-500 bg-slate-100 dark:bg-white/10 px-2 py-0.5 rounded text-center min-w-[3rem]">
                                ...
                                {filteredStudents.length > 0 ? filteredStudents.map(student => {
                                    ...
                                    return (
                                        <li
                                            ...
                                            className={`flex items-center gap-3 p-2.5 sm:p-3 rounded-lg sm:rounded-xl transition-all mb-1 ${
                                        ...
                                        >
                                            ...
                                            <span className={`text-xs sm:text-sm font-medium flex-grow select-none ${isSelected ? 'text-blue-700 dark:text-blue-300' : 'text-slate-700 dark:text-slate-300'}`}>
                                                {studentName}
                                            </span>
                                        </li>
                                    );
                    ...
                    `}>
                        ...
                        {hasQuizzes ? (
                            <div className={`${glassPanel} flex-1 p-4 sm:p-6 overflow-y-auto custom-scrollbar min-h-0`}>
                                <div className="space-y-4 sm:space-y-6">
                                    
                                    {/* Max Attempts */}
                                    <div className="flex items-center justify-between p-3 sm:p-4 bg-slate-50/50 dark:bg-white/5 rounded-xl border border-slate-200/50 dark:border-white/5">
                                        ...
                                        <input
                                            ...
                                            className={`${glassInput} w-14 sm:w-16 text-center !p-1.5 sm:!p-2`}
                                        />
                                    </div>
                                    ...
                                    <div>
                                        ...
                                        <div className={`space-y-0.5 sm:space-y-1 pl-3 sm:pl-4 border-l-2 border-slate-200 dark:border-white/10 transition-all duration-300 ${quizSettings.enabled ? 'opacity-100' : 'opacity-40 pointer-events-none grayscale'}`}>
                                            {[
                ...
                {/* Footer */}
                <footer className="flex-shrink-0 flex flex-wrap justify-between items-center pt-4 sm:pt-6 border-t border-slate-200/60 dark:border-white/5 mt-2 gap-3 sm:gap-4">
                    <button
