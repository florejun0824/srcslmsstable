            ...
            <Dialog.Panel as={motion.div} variants={modalVariants} initial="hidden" animate="visible" exit="exit" className="relative bg-neumorphic-base rounded-2xl shadow-neumorphic w-full max-w-5xl z-10 flex flex-col h-full md:h-[90vh] md:max-h-[700px] overflow-hidden">
                ...
                <header className="flex justify-between items-center p-4 sm:p-5 bg-neumorphic-base flex-shrink-0 z-10 border-b border-neumorphic-shadow-dark/10">
                    <div className="flex items-center gap-3 overflow-hidden">
                        ...
                        {currentLesson.studyGuideUrl && (<a href={currentLesson.studyGuideUrl} download target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-2 px-3 py-1.5 text-xs sm:text-sm font-semibold text-slate-700 bg-neumorphic-base rounded-full shadow-neumorphic active:shadow-neumorphic-inset transition-shadow duration-150 ease-out whitespace-nowrap"><ArrowDownTrayIcon className="h-4 w-4 sm:h-5 sm:w-5" /><span className="hidden sm:inline">Study Guide</span></a>)}
                    </div>
                ...
                <footer className="grid grid-cols-3 items-center p-4 bg-neumorphic-base border-t border-neumorphic-shadow-dark/10 flex-shrink-0 z-10">
                    ...
                    <div className="flex items-center gap-2 justify-center">
                        {pageData?.type === 'diagram-data' && (
                            <>
                                <button onClick={() => lessonPageRef.current?.addImage()} className="p-2.5 rounded-full text-slate-600 bg-neumorphic-base shadow-neumorphic active:shadow-neumorphic-inset transition-all duration-150 ease-out" title="Add Image"><PhotoIcon className="h-5 w-5" /></button>
                                <button onClick={() => lessonPageRef.current?.addLabel()} className="p-2.5 rounded-full text-slate-600 bg-neumorphic-base shadow-neumorphic active:shadow-neumorphic-inset transition-all duration-150 ease-out" title="Add Label"><CursorArrowRaysIcon className="h-5 w-5" /></button>
                                <button onClick={() => lessonPageRef.current?.finalizeDiagram()} className="flex items-center gap-2 px-4 py-2 rounded-full text-sm font-semibold text-slate-700 bg-neumorphic-base shadow-neumorphic active:shadow-neumorphic-inset transition-all duration-150 ease-out" disabled={isFinalizing}>
                        ...
                        {pageData?.type === 'diagram' && (
                            <button onClick={handleRevertDiagramToEditable} className="flex items-center gap-2 px-4 py-2 rounded-full text-sm font-semibold text-slate-700 bg-neumorphic-base shadow-neumorphic active:shadow-neumorphic-inset transition-all duration-150 ease-out"><PencilSquareIcon className="h-5 w-5" />Re-edit Diagram</button>
                        )}
                    </div>
