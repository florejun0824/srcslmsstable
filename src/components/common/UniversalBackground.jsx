...
const UniversalBackground = () => {
   const { activeOverlay, monetTheme, themeMode, isDarkMode } = useTheme();

   // Helper to check if we should render heavy effects
   const showEffects = themeMode === 'full';

   // Light mode: soft pastel gradient base; Dark mode: monet seed color
   const bgStyle = isDarkMode
      ...
      : {
         backgroundColor: monetTheme.seedHex,
         backgroundImage: `
            radial-gradient(at 0% 0%, rgba(${monetTheme.rgbString}, 0.15) 0px, transparent 50%),
            radial-gradient(at 100% 0%, rgba(${monetTheme.rgbString}, 0.1) 0px, transparent 50%),
            radial-gradient(at 50% 100%, rgba(${monetTheme.rgbString}, 0.08) 0px, transparent 50%)
        `
      };
   ...
   return (
      ...
      >
         ...
         {/* NOISE TEXTURE - Keep this for both modes as it is static */}
